...
router.beforeEach((to, from, next) => {
    ...
    if ((auth && to.path.indexOf('/auth/') !== -1) || (!auth && to.meta.auth) || (articleId && !store.getters.getArticleById(articleId)) || (paramUser && paramUser !== user && !store.getters.getArticleByUid(null, paramUser).length)) {
        next('/')
    } else {
        next()
    }
